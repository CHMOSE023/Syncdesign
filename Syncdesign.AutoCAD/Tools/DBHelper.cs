...
namespace Syncdesign.AutoCAD.Tools;
...
public static class DBHelper
{
    ...
    /// 设置命名字典数据
    /// </summary> 
    public static void SetNODData(string name, string key, Xrecord xrecord, Database acCurDb = null)
    {
        acCurDb = acCurDb ?? Application.DocumentManager.MdiActiveDocument.Database;
        using (var trans = acCurDb.TransactionManager.StartOpenCloseTransaction())
        {
            var nod = trans.GetObject(acCurDb.NamedObjectsDictionaryId, OpenMode.ForRead) as DBDictionary;
            /* 如果没有字典 则生成一个字典*/
            if (!nod.Contains(name))
            {
                // StartOpenCloseTransaction 更安全执行事务方式
                using (var innerTrans = acCurDb.TransactionManager.StartOpenCloseTransaction())
                {
                    var nodW = trans.GetObject(acCurDb.NamedObjectsDictionaryId, OpenMode.ForWrite) as DBDictionary;
    ...
    /// 获取命名字典数据
    /// </summary> 
    public static Xrecord GetNODData(string name, string key, Database acCurDb = null)
    {
        acCurDb = acCurDb ?? Application.DocumentManager.MdiActiveDocument.Database;
